...
// Security features
// NOTE: IP limiting is not currently implementable because SpacetimeDB does not expose
// client IP addresses to reducers. The infrastructure is in place for when this becomes available.
pub const ENABLE_IP_LIMITING: bool = false;      // Limit 1 active ballot per IP address (NOT IMPLEMENTED - SpacetimeDB limitation)
...
pub struct Vote {
    ...
    // Security options
    pub limit_by_ip: bool,      // If true, limit 1 active ballot per IP
}
...
pub fn create_vote(
    ...
    let use_ip_limit = limit_by_ip.unwrap_or(false);
    
    if use_ip_limit {
        return Err("IP-based ballot limiting is not implemented yet (SpacetimeDB does not expose client IP addresses)".to_string());
    }
