 ...
 */
export class MjSummaryTableHandle<TableName extends string> implements __TableHandle<TableName> {
  // phantom type to track the table name
  readonly tableName!: TableName;
  tableCache: __TableCache<MjSummary>;

  constructor(tableCache: __TableCache<MjSummary>) {
  ...
  optionId = {
    ...
    find: (col_val: number): MjSummary | undefined => {
      for (let row of this.tableCache.iter()) {
        if (__deepEqual(row.optionId, col_val)) {
          return row;
        }
      }
    },
