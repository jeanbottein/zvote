...
export const spacetimeDB = {
  ...
  async connect(token?: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const MODULE_NAME = config.backendSpacetimeDbName;
...
// Auto-connect when loaded, but only if using SpacetimeDB backend
if (typeof window !== 'undefined') {
  const config = getConfig();
  if (config.backendType === 'spacetime') {
    console.log('[spacetimeClient.ts] Auto-connecting to SpacetimeDB...');
    spacetimeDB.connect().catch(console.error);
  } else {
    console.log('[spacetimeClient.ts] Skipping auto-connect (using GraphQL backend)');
  }
}
